...
async def handle_pay(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(PaymentProof.waiting_for_screenshot)  # Устанавливаем состояние
...
# Исправленный фильтр: StateFilter вместо FSMContext()
...
# Обработчик отмены для состояния ожидания скриншота
...
async def handle_confirm(callback: CallbackQuery):
    ...
    await callback.message.bot.send_message(
        ...
        text=f"✅ Ваша оплата по счёту `{bill.id}` подтверждена!\n"
             f"Сумма: {debt.amount:.2f} ₽\n"
             f"Остаток по счёту: {bill.amount_left:.2f} ₽"
    )
    await callback.answer()
